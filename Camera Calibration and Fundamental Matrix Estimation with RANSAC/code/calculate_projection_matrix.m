...
row = size(Points_2D, 1);
A = zeros(row * 2, 11);
B = zeros(row * 2, 1);
for i = 1 : row
    ii = i * 2 - 1;
    A(ii, :) = [Points_3D(i, :), 1, 0, 0, 0, 0, -Points_2D(i, 1)*Points_3D(i, :)];
    A(ii + 1, :) = [0, 0, 0, 0, Points_3D(i, :), 1, -Points_2D(i, 2)*Points_3D(i, :)];
    B(ii, 1) = Points_2D(i, 1);
    B(ii + 1, 1) = Points_2D(i, 2);
end
