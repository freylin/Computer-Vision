...
% CLASSIFIER = 'support vector machine with chisqr';
...
%         if (~exist('train_image_feats.mat', 'file') || ~exist('test_image_feats.mat', 'file'))
%             train_image_feats = get_bags_of_sifts(train_image_paths);
%             test_image_feats  = get_bags_of_sifts(test_image_paths);
...
%         load('test_image_feats.mat');
    ...
    case 'fisher'
        if (~exist('means.mat', 'file') || ~exist('covariances.mat', 'file') || ~exist('priors.mat', 'file'))
            vocab_size = 10;
            build_gmm(train_image_paths, vocab_size);
        end
        
        train_image_feats = get_fisher(train_image_paths);
        test_image_feats  = get_fisher(test_image_paths);
%           if (~exist('train_image_feats.mat', 'file') || ~exist('test_image_feats.mat', 'file'))
%               train_image_feats = get_fisher(train_image_paths);
%               test_image_feats  = get_fisher(test_image_paths);
%               save('train_image_feats.mat', 'train_image_feats');
...
switch lower(CLASSIFIER)    
    ...
    case 'support vector machine with chisqr'
        % YOU CODE svm_classify.m 
        predicted_categories = svm_classify_ChiSqr(train_image_feats, train_labels, test_image_feats);
