...
[train_image_paths_ori, test_image_paths_ori, train_labels_ori, test_labels_ori] = ...
    get_image_paths(data_path, categories, num_train_per_cat);
%   train_image_paths_ori  1500x1   cell      
%   test_image_paths_ori   1500x1   cell           
%   train_labels_ori       1500x1   cell         
%   test_labels_ori        1500x1   cell          
...
image_paths_merged_ori = [train_image_paths_ori; test_image_paths_ori];
image_labels_merged_ori = [train_labels_ori; test_labels_ori];
N = size(image_paths_merged_ori, 1);
for i = 1 : iterations
    image_paths_merged = image_paths_merged_ori;
    image_labels_merged = image_labels_merged_ori;
    
    sample_index = randsample(1:N, 100);
    train_image_paths = image_paths_merged(sample_index);
    train_labels = image_labels_merged(sample_index);
    
    % remove selected images, avoid overlap    
    image_paths_merged(sample_index) = [];
    image_labels_merged(sample_index) = [];
    
    sample_index = randsample(1:(N-100), 100);
    test_image_paths = image_paths_merged(sample_index);
    test_labels = image_labels_merged(sample_index);
...
%             end
    ...
    accuracy(i, 1) = create_results_webpage( train_image_paths, ...
                            test_image_paths, ...
                            train_labels, ...
                            test_labels, ...
                            categories, ...
    ...
    disp(i);
    fprintf(     'Average accuracy is %.3f\n', mean(accuracy(1:i)));
    fprintf(     'Standard deviation is %.3f\n', std(accuracy(1:i)));               
end
