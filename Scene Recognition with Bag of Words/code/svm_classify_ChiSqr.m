...
function predicted_categories = svm_classify_ChiSqr(train_image_feats, train_labels, test_image_feats)
...
confidences = zeros(num_categories, N);
W = zeros(num_categories, N);
B = zeros(num_categories, 1);
LAMBDA = 50;
global K
K = chiSquared(train_image_feats, train_image_feats);
K2 = chiSquared(test_image_feats, train_image_feats);
for i = 1 : num_categories
    disp(i);
    labels = -ones(N, 1);
    labels(strcmp(categories{i}, train_labels)) = 1;
    [w, b] = primal_svm(0, labels, LAMBDA);
    W(i, :) = w;
    B(i, 1) = b;
end
confidences = W * K2 + B;
