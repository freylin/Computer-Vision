...
% features_pos = get_interesting_positive_features( train_path_pos, feature_params );
...
% trianing data.
Mining_hard_negatives = false;
if (Mining_hard_negatives)
    num_samples = 10000;
    if (~exist('features_hard_neg.mat', 'file'))
        features_hard_neg = get_hard_negative_features(non_face_scn_path, w, b, feature_params, num_samples);
        save('features_hard_neg.mat', 'features_hard_neg');
    else
        load('features_hard_neg.mat');
    end
    X = [X; features_hard_neg];
    Y = [Y; -ones(size(features_hard_neg, 1), 1)];
    [w, b] = vl_svmtrain(X', Y', lambda);
end
