...
for i = 1 : size(x,1)
    % get the cell
    mag_cell = mag(y(i)-f2 : y(i)+f2-1, x(i)-f2 : x(i)+f2-1);
    dir_cell = dir(y(i)-f2 : y(i)+f2-1, x(i)-f2 : x(i)+f2-1);
    feature = [];
    for j = 1 : 4
        jj = (j-1) * f4 + 1;
        for k = 1 : 4
            kk = (k-1) * f4 + 1;
            mag_cell2 = mag_cell(jj : jj+f4-1, kk : kk+f4-1);
            dir_cell2 = dir_cell(jj : jj+f4-1, kk : kk+f4-1);
            % orientation
            ori = floor((dir_cell2 + 180) / 45) + 1;
            bin = zeros(8, 1);
            for l = 1 : (f4)^2
                ori2 = ori(indi(l), indj(l));
                if ori2 > 8
                    ori2 = 8;
                end
                % add to bin
                bin(ori2) = bin(ori2) + mag_cell2(indi(l), indj(l)); 
            end
            feature = [feature; bin];
        end
    end
